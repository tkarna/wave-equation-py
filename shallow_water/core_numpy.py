...
def run(grid, initial_elev_func, bathymetry_func,
    ...
    # state variables
    dtype = numpy.float64
    elev = numpy.zeros(grid.T_shape, dtype=dtype)
    u = numpy.zeros(grid.U_shape, dtype=dtype)
    v = numpy.zeros(grid.V_shape, dtype=dtype)

    # potential vorticity
    q = numpy.zeros(grid.F_shape, dtype=dtype)

    # energy
    ke = numpy.zeros(grid.T_shape, dtype=dtype)
    pe = numpy.zeros(grid.T_shape, dtype=dtype)

    # bathymetry
    h = numpy.zeros(grid.T_shape, dtype=dtype)
    ...
    def rhs(u, v, elev):
        ...
        dudy = numpy.zeros(grid.F_shape, dtype=dtype)  # F point (nx+1, nx+1)
        dudy[:, 1:-1] = (u[:, 1:] - u[:, :-1])/grid.dy
        dudy[:, 0] = (u[:, 0] - u[:, -1])/grid.dy
        dudy[:, -1] = dudy[:, 0]

        dvdx = numpy.zeros(grid.F_shape, dtype=dtype)  # F point (nx+1, nx+1)
        ...
        if not use_vector_invariant_form:
            ...
            dudx = numpy.zeros((grid.nx + 2, grid.ny))  # T point extended for BC
            dudx[1:-1, :] = (u[1:, :] - u[:-1, :])/grid.dx
            dudx[0, :] = (u[0, :] - u[-1, :])/grid.dx
            dudx[-1, :] = dudx[0, :]
            uux[:, :] = numpy.where(u > 0, dudx[:-1, :], dudx[1:, :]) * u
            dvdy = numpy.zeros((grid.nx, grid.ny + 2))  # T point extended for BC
            ...
            H_at_f = numpy.zeros(grid.F_shape, dtype=dtype)
            ...
            qhv = numpy.zeros(grid.U_shape, dtype=dtype)
            qhv[:-1, :] += q_a[:, :] * hv[:, 1:]
            qhv[-1, :] += q_a[0, :] * hv[0, 1:]
            qhv[1:, :] += q_b[:, :] * hv[:, 1:]
            qhv[0, :] += q_b[-1, :] * hv[-1, 1:]
            ...
            qhv[:-1, :] += q_d[:, :] * hv[:, :-1]
            qhv[-1, :] += q_d[0, :] * hv[0, :-1]
            qhu = numpy.zeros(grid.V_shape, dtype=dtype)
            qhu[:, :-1] += q_g[:, :] * hu[1:, :]
            qhu[:, -1] += q_g[:, 0] * hu[1:, 0]
            qhu[:, :-1] += q_d[:, :] * hu[:-1, :]
            qhu[:, -1] += q_d[:, 0] * hu[:-1, 0]
            ...
            # gradient of ke
            dkedx = numpy.zeros(grid.U_shape, dtype=dtype)
            dkedx[1:-1, :] = (ke[1:, :] - ke[:-1, :])/grid.dx
            dkedx[0, :] = (ke[0, :] - ke[-1, :])/grid.dx
            dkedx[-1, :] = dkedx[0, :]
            dkedy = numpy.zeros(grid.V_shape, dtype=dtype)
    ...
    if runtime_plot:
        ...
        ax = ax_list[0]
        vmax = 0.15
        img1 = ax.pcolormesh(grid.x_u_1d, grid.y_v_1d, elev.T, vmin=-vmax, vmax=vmax, cmap=plt.get_cmap('RdBu_r', 61))
        plt.colorbar(img1, label='Elevation')
        ax.set_aspect('equal')
        ax = ax_list[1]
        vmax = 0.7
        u_at_t = 0.5 * (u[1:, :] + u[:-1, :])
        img2 = ax.pcolormesh(grid.x_u_1d, grid.y_v_1d, u_at_t.T, vmin=-vmax, vmax=vmax, cmap=plt.get_cmap('RdBu_r', 61))
    ...
    for i in range(nt+1):
        ...
        if t >= next_t_export:
            ...
            H = elev + h
            total_ke = float(numpy.sum(H * ke)) * grid.dx * grid.dy
            total_pe = float(numpy.sum(pe)) * grid.dx * grid.dy
            total_e = total_ke + total_pe
            total_v = float(numpy.sum(H)) * grid.dx * grid.dy
            if diff_e is None:
                initial_e = total_e
                initial_v = total_v
